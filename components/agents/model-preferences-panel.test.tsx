...
import type { ModelPreference } from "@/lib/api/ai-models";
...
describe("ModelPreferencesPanel", () => {
  ...
  it("saves the model for one task only", async () => {
    ...
    await waitFor(() => expect(setModelPreference).toHaveBeenCalledWith(tenantId, "Draft", "otro/modelo"));
    expect(
  ...
  it("clears the override and re-reads what the default now is", async () => {
    ...
    await waitFor(() => expect(clearModelPreference).toHaveBeenCalledWith(tenantId, "Classify"));
    expect(await screen.findByDisplayValue("deepseek/deepseek-v4-flash")).toBeInTheDocument();
  });
